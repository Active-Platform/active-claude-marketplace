...
- **Get a preview or download link** for a document — produce a web-viewer preview URL (to open in a
  browser) or a direct download URL for the document's active version.
...
- **To put entities under one group, use `parentClientId` — not `groupName`.** To nest an entity under an
  existing client group, resolve that group's id first (combined or group search) and pass it as
  `parentClientId`. `groupName` does **not** join an existing group by name — it names a brand-new group
  built around that single entity, so reusing the same `groupName` across several creates makes a separate
  one-member group each time, never one shared group.
...
- **Link record names into the app.** A client, group or binder result — from a `…-get` or a
  ...
  record that has no `webUrl`. To point inside a binder, append one of these fixed sub-paths to that
