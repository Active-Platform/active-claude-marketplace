...
- `subagent.status` tells the recipes that spawn `wp-discover` — `wp-find-and-file`,
  `wp-client-queries-initial-request`, `wp-client-queries-file-and-followup` — *where* the sweep
  runs, never *whether* it runs. On `unreachable`, run the discover leg inline in the main context
  instead of spawning a worker that will come back empty, say why, and point at `wp-setup` for the
  override that restores the worker. A spawned worker that reports no Active tools means the same
  thing, whatever the manifest says: re-run that leg inline rather than reporting a dead end.
