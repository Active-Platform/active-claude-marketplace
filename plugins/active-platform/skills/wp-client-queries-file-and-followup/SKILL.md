...
2. **File the response(s) — input switch (subagent scan via `subagent_type: wp-discover`, or inline
   when `subagent.status` is `unreachable` or the worker returns no Active tools; inline file + epilogue):**
